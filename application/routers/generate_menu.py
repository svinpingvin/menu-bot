...
async def handle_generate_menu_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("На сколько дней сгенерировать меню?", reply_markup=days_keyboard())
...
async def handle_days_selection(
        callback,
        generate_menu_interactor: FromDishka[GenerateMenuInteractor]
) -> None:
    ...
    try:
        ...
        await callback.message.edit_text(f"Сгенерированное меню:\n\n{formatted_menu}", reply_markup=None)
    except Exception as e:
        await callback.message.edit_text(f"Ошибка при генерации меню: {e}", reply_markup=None)
